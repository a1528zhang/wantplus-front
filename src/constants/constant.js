//首页获取所有商品
export const GET_ALL_COMMODITY = "GET_ALL_COMMODITY";

//
export const DISPLAY_COMMODITY_DETAIL = "DISPLAY_COMMODITY_DETAIL";

//获取所有商店的列表
export const GET_ALL_SHOP = "GET_ALL_SHOP";

export const HIDDEN_COMMODITY_DETAIL = "HIDDEN_COMMODITY_DETAIL";
export const GET_COMMODITY_DETAIL_DATA = "GET_COMMODITY_DETAIL_DATA";
export const GET_DETAIL_DATA = "GET_DETAIL-DATA";
export const GET_COMMODITY_DETAIL_RECOMMEND_DATA = "GET_COMMODITY_DETAIL_RECOMMEND_DATA";
export const RECOMMEND_COMMODITY_DETAIL = "RECOMMEND_COMMODITY_DETAIL";

export const ADD_COMMODITY_URL = "ADD_COMMODITY_URL";

export const SIGN_IN = "SIGN_IN";
export const NAV_GET_FOCUS = "NAV_GET_FOCUS";
export const SHOW_COMMODITY_RECOMMEND = "SHOW_COMMODITY_RECOMMEND";
export const FROM_RECOMMEND_GET_DETAILS_DATA = "FROM_RECOMMEND_GET_DETAILS_DATA";



export const SHOP_DATA = {
    exceptionName: "请求成功",
    state: 1,
    data: [
        {
            shopId: 1,
            shopName: "家居"
        },
        {
            shopId: 2,
            shopName: "服饰"
        },
        {
            shopId: 3,
            shopName: "数码"
        }

    ]
}

export const DETAIL_DATA = {
    exceptionName: "请求成功",
    state: 1,
    data: {
        commodityId: 28,
        commodityUploadUser: 1,
        commodityUploadUserName: "az",
        commodityLike: 0,
        commodityCollection: 0,
        commodityName: "asdasd",
        commodityPrice: 321,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic9.jpg",
        shopId: 1,
        shopName: "精品店",
        shopUrl: "www.baidu.com",
        shopIcon: "https://ss0.baidu.com/6ONWsjip0QIZ8tyhnq/it/u=1696411074,2476960811&fm=58",
        platform: "淘宝",
        userBrief: [
            {
                userId: 5,
                headImg: "http://7xpbw2.com1.z0.glb.clouddn.com/3_20151229222445_bag_S.jpg"
            },
            {
                userId: 4,
                headImg: "http://7xpbw2.com1.z0.glb.clouddn.com/5_20151229222549_glass_S.jpg"
            },
            {
                userId: 3,
                headImg: "http://7xpbw2.com1.z0.glb.clouddn.com/4_20151229222536_glassbag_S.jpg"
            },
            {
                userId: 2,
                headImg: "http://7xpbw2.com1.z0.glb.clouddn.com/4_20151229222515_tea_S.jpg"
            }
        ]
    }
}

export const RECOMMEND_DATA = [
    {
        userId: 100,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic9.jpg"
    },
    {
        userId: 101,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic8.jpg"
    },
    {
        userId: 102,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic7.jpg"
    },
    {
        userId: 103,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic6.jpg"
    },
    {
        userId: 104,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic5.jpg"
    }
]

export const USER_HEAD_IMG = [
    {
        userId: 105,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/1.jpg"
    },
    {
        userId: 106,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/2.png"
    },
    {
        userId: 107,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/3.jpg"
    },
    {
        userId: 108,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/4.jpg"
    },
    {
        userId: 109,
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/5.jpg"
    }
]

export const SHOP_LIST = [
    {
        userId: 110,
        shopName:"张黎的手",
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic1.jpg"
    },
    {
        userId: 111,
        shopName:"张黎的脚",
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic2.jpg"
    },
    {
        userId: 112,
        shopName:"张黎的屁股",
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic3.jpg"
    },
    {
        userId: 113,
        shopName:"张黎的眼睛",
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic4.jpg"
    },
    {
        userId: 114,
        shopName:"张黎的鼻子",
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic5.jpg"
    },
    {
        userId: 115,
        shopName:"张黎的耳朵",
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic6.jpg"
    },
    {
        userId: 116,
        shopName:"张黎的鼻屎",
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic10.jpg"
    },
    {
        userId: 117,
        shopName:"张黎的鞋子",
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic8.jpg"
    },
    {
        userId: 118,
        shopName:"张黎的内裤",
        commodityImg: "http://7xpmhh.com1.z0.glb.clouddn.com/pic9.jpg"
    }
]


